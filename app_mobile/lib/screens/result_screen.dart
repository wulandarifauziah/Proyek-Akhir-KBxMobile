...
import 'package:uuid/uuid.dart';
import '../models/prediction_model.dart';
import 'history_screen.dart'; // Impor HistoryItem & HistoryService
import '../config/routes.dart';
...
class _ResultScreenState extends State<ResultScreen> {
  ...
  void initState() {
    super.initState();
    // Panggil fungsi penyimpanan setelah build selesai
    Future.microtask(() => _saveToHistory());
  }
  ...
  Future<void> _saveToHistory() async {
    ...
    try {
      ...
      if (imagePath == null) {
        if (widget.imageBytes != null && widget.imageBytes!.isNotEmpty) {
          // Jika imagePath null, gunakan imageBytes (biasanya dari galeri web)
          imageBase64 = base64Encode(widget.imageBytes!);
        } else {
          debugPrint(
            'Tidak menyimpan ke riwayat: imagePath dan imageBytes tidak tersedia.',
      ...
      final HistoryItem newItem = HistoryItem(
        ...
        mineralName: widget.mineralName,
        confidence: widget.confidence,
        imagePath: imagePath, // Path lokal dari kamera/galeri
        imageBase64: imageBase64, // Base64 dari web atau galeri tanpa path
        timestamp: DateTime.now(),
      );
      ...
      debugPrint('✅ Hasil prediksi disimpan ke riwayat.');
    } catch (e) {
      debugPrint('Gagal menyimpan ke riwayat: $e');
      if (mounted && kDebugMode) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    bool isConfident = widget.confidence >= 0.60;
    bool hasAlternatives = widget.alternatives.isNotEmpty;
