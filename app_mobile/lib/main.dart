...
import 'package:firebase_core/firebase_core.dart';
import 'firebase_options.dart';
// 🎯 DITAMBAHKAN: Import untuk kamera
import 'package:camera/camera.dart';
// import 'screens/camera_guidance_screen.dart'; // Tidak perlu jika 'cameras' global

// 🎯 DEKLARASI GLOBAL UNTUK KAMERA (Diakses oleh CameraGuidanceScreen)
late List<CameraDescription> cameras;

void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  // 🎯 PERBAIKAN 1: Mengganti initializeCameras() dengan logika inisialisasi yang benar
  try {
    cameras = await availableCameras();
  } on CameraException catch (e) {
    print('Error getting available cameras: $e');
    cameras = []; // Pastikan list tetap diinisialisasi
  }
...
class _MineralScannerAppState extends State<MineralScannerApp> {
  ...
  ThemeData _buildTheme(ColorScheme colorScheme) {
    ...
    return ThemeData(
      ...
      scaffoldBackgroundColor: colorScheme.surface,
      // 🎯 PERBAIKAN 2 & 3: CardTheme dan Opasitas
      cardTheme: CardThemeData(
        // Ganti CardThemeData menjadi CardTheme
        elevation: 6,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        color: colorScheme.brightness == Brightness.dark
            ? colorScheme.surface.withOpacity(
                0.92,
              ) // Ganti .withValues(alpha:...) dengan .withOpacity(...)
            : colorScheme.surface,
